...
def test_dfs_category_math(curriculum_graph):
    ...
    assert {node["id"] for node in result_graph["nodes"]} == set(expected_nodes)
    assert {(edge["from"], edge["to"]) for edge in result_graph["edges"]} == {
...
def test_extract_subgraph_math_category():
    ...
    assert {
        (node["id"], tuple(node["data"]["category"]))
        for node in subgraph["nodes"]
    } == {
        (node["id"], tuple(node["data"]["category"]))
        for node in graph_example["nodes"]
    }
    assert {(edge["from"], edge["to"]) for edge in subgraph["edges"]} == {
